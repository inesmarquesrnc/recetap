...
const RecetasConst = props => (
    <tr>
        <td>{props.ensaladas._id}</td>
        <td>
            <Link to={"/edit/" + props.ensaladas._id}>Edit</Link>
        </td>
    </tr>
)

export class Ensaladas extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:5000/acompaniamiento/get')
            .then(res => {
